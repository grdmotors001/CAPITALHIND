...
const TOKEN_TTL = process.env.JWT_TOKEN_TTL || '12h';
...
// Generic auth for `users` table roles other than admin.
// Pass the roles this endpoint accepts.
...
export function requireCashierAuth(req, res) {
  const header = req.headers['authorization'] || '';
  const token = header.startsWith('Bearer ') ? header.slice(7) : null;
  if (!token) { sendError(res, 401, 'Not logged in. Please login again.'); return null; }
  try {
    const decoded = jwt.verify(token, JWT_SECRET);
    if (decoded.type !== 'staff_user' || decoded.role !== 'cashier' || !decoded.staff_user_id) {
      sendError(res, 401, 'Invalid cashier session. Please login again.'); return null;
    }
    return decoded;
