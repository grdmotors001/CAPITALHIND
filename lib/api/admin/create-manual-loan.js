...
export default async function handler(req,res){
 ...
 try{
  ...
  if(!dealer){
   const ins=await supabase.from('dealer_master').insert({dealer_name:String(b.dealer_name).trim(),dealer_code:String(b.dealer_code||'').trim()||null,contact_phone:String(b.dealer_phone||'').trim()||null,is_active:true}).select('id,dealer_name,dealer_code').single();
   if(ins.error) throw ins.error; dealer=ins.data;
  }
  let model;
  ...
  const ar=await supabase.from('loan_applications').insert({
   ...
   chassis_no:b.chassis_no||null,engine_no:b.engine_no||null,disbursement_date:b.disbursement_date||null,
   disbursed_amount:b.disbursed_amount!==''?Number(b.disbursed_amount):loanAmount,
   hypothecation:b.hypothecation||null,loan_type:b.loan_type||null,remarks:b.remarks||null,
