...
export default async function handler(req, res) {
  ...
  try {
    ...
    // GRD's login_id/mobile is the most reliable cross-system identifier.
    // Resolve it through CHFPL dealer_users first, then use dealer_id to
    // resolve dealer_master. This avoids relying on dealer-code/name parity.
    const identityPhones = [dealerLoginId, dealerMobile].map(digits).filter(Boolean);
    if (identityPhones.length) {
      // Phone values may be stored as 10 digits on one side and +91/other
      // formatting on the other. Compare normalized digits in application code.
      const { data: users, error } = await supabase
        .from('dealer_users')
        .select('id, dealer_id, full_name, phone, is_active')
        .limit(2000);
      if (error) throw error;
      dealerUser = (users || []).find(u => {
        const p = digits(u.phone);
        return p && identityPhones.some(x => p === x || p.slice(-10) === x.slice(-10));
      }) || null;

      if (dealerUser?.dealer_id) {
        const { data: dealerByUserId, error: dealerByUserIdErr } = await supabase
          .from('dealer_master')
          .select('id, dealer_name, dealer_code')
          .eq('id', dealerUser.dealer_id)
          .maybeSingle();
        if (dealerByUserIdErr) throw dealerByUserIdErr;
        dealer = dealerByUserId;
      }
    }

    if (dealerCode && !dealer) {
      const { data, error } = await supabase
        .from('dealer_master')
        .select('id, dealer_name, dealer_code')
    ...
    if (!dealer) {
      return sendError(res, 404, 'CHFPL dealer not found for supplied code/name/mobile/login_id');
    }

    if (!dealerUser && dealerLoginId) {
      const { data, error } = await supabase
        .from('dealer_users')
        .select('id, dealer_id, full_name, phone, is_active')
