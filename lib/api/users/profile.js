...
export default async function handler(req, res) {
  ...
  try {
    ...
    const id = isDealer ? session.dealer_user_id : session.user_id;
    const table = isDealer ? 'dealer_users' : 'users';

    if (req.method === 'GET') {
      const { data, error } = await supabase.from(table).select(isDealer ? dealerFields : fields).eq('id', id).maybeSingle();
    ...
    const { data: duplicate } = await supabase.from(table).select('id').eq('phone', payload.phone).neq('id', id).limit(1).maybeSingle();
    if (duplicate) return sendError(res, 409, 'This mobile number is already registered.');

    const { data, error } = await supabase.from(table).update(payload).eq('id', id).select(isDealer ? dealerFields : fields).maybeSingle();
    if (error) {
