...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [{ data: fes, error: feErr }, { data: receipts, error: rErr }, { data: handovers, error: hErr }] = await Promise.all([
      ...
      supabase.from('loan_receipts').select('id, entered_by, receipt_no, receipt_date, amount, created_at, loan_applications(application_no, vehicle_no, customer_profiles(full_name, phone))').eq('collection_source','field_executive').order('created_at',{ascending:false}).limit(1000),
