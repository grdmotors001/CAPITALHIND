...
// --- Collection & Risk -------------------------------------------------------
async function collectionRiskRequest(method, params, body) {
  const token = getAdminToken();
  const qs = params ? `?${new URLSearchParams(params).toString()}` : '';
  const response = await fetch(`${AUTH_API_BASE}/collection-risk${qs}`, {
    method,
...
export const listNpaAgeing = () => collectionRiskRequest('GET', { view: 'npa' });
export const listRestructureRequests = (status = 'All') => collectionRiskRequest('GET', { view: 'requests', status });
export const getRiskConfig = () => collectionRiskRequest('GET', { view: 'config' });
export const updateRiskConfig = (patch) => collectionRiskRequest('POST', null, { action: 'update-config', ...patch });
export const createRestructureRequest = (body) => collectionRiskRequest('POST', null, body);
export const decideRestructureRequest = (id, status, remarks) => collectionRiskRequest('PATCH', null, { id, status, remarks });
