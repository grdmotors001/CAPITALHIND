            ...
            <tbody>
              {loading ? (
                ...
                <tr key={app.id}>
                  ...
                  <td><span className={`role-pill ${app.tvr_status==='verified'?'field_executive':'staff'}`}>TVR: {app.tvr_status || 'pending'}</span></td>
                  <td className="actions-cell">
                    <button className="admin-btn small" disabled={app.tvr_status !== 'verified'} onClick={() => selectApplication(app.id)}>{app.tvr_status === 'verified' ? 'Create Loan' : 'TVR Pending'}</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <section className="admin-card staff-list-card" style={{ marginTop: 20 }}>
          ...
          <form className="staff-form" onSubmit={handleCreate}>
            ...
            <div className="admin-alert" style={{ marginTop: 14 }}>
              TVR status: <strong>{selected.tvr_status || 'pending'}</strong>. &nbsp;
