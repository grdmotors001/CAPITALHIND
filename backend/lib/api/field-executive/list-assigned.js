...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data: assignedRows, error: assignedErr } = await supabase
      ...
      .select(`
        id, application_no, loan_account_no, application_status, assigned_at, case_status, vehicle_no, grd_dealer_id, emi_amount,
    ...
    if (me?.full_name && me.role === 'field_executive' && me.is_active) {
      ...
      if ((sameNameUsers || []).length === 1 && sameNameUsers[0].id === session.user_id) {
        const { data, error } = await supabase
          .from('loan_applications')
          .select(`
            id, application_no, loan_account_no, application_status, assigned_at, case_status, vehicle_no, emi_amount,
            customer_profiles ( full_name, phone, address, city ),
            vehicle_model_master ( model_name ),
            fi_reports ( recommendation, remarks, created_at )
          `)
    ...
    const applications = (data || []).map((row) => {
      ...
      return {
        ...
        grd_dealer_id: row.grd_dealer_id ?? null,
