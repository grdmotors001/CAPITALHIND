...
export default async function handler(req, res) {
  ...
  try {
    ...
    // These metadata columns are optional for backwards compatibility. The
    // receipt itself is saved using only the base loan_receipts columns, then
    // metadata is added when the newer migration is present.
    await supabase.from('loan_receipts').update({ collection_source: 'field_executive', collected_at: collectedAt }).eq('id', receipt.id);
    ...
    return res.status(200).json({
      ...
      receipt: {
        ...
        customer_phone: loan.customer_profiles?.phone || null,
        application_no: loan.application_no || null,
        loan_account_no: loan.loan_account_no || null,
        vehicle_no: loan.vehicle_no || null,
        payment_mode: 'cash',
