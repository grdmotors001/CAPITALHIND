// POST /api/admin/upload-kyc-document
...
const ALLOWED_DOC_TYPES = ['pan','aadhaar_front','aadhaar_back','photo','address_proof','income_proof','bank_statement','other'];
const ALLOWED_MIME_TYPES = ['image/jpeg','image/png','application/pdf'];
const MAX_FILE_SIZE_BYTES = 5 * 1024 * 1024;
const BUCKET = 'kyc-documents';
function parseForm(req) { const form = formidable({ maxFileSize: MAX_FILE_SIZE_BYTES, multiples: false }); return new Promise((resolve,reject) => form.parse(req,(err,fields,files) => err ? reject(err) : resolve({fields,files}))); }
export default async function handler(req,res) {
  if (!methodGuard(req,res,'POST')) return;
  const session = requireAdminAuth(req,res); if (!session) return;
  let fields, files;
  try { ({fields,files}=await parseForm(req)); } catch { return sendError(res,422,'File too large or upload failed (max 5 MB)'); }
  ...
  if (!loanApplicationId || !customerId) return sendError(res,422,'loan_application_id and customer_id are required');
  if (!ALLOWED_DOC_TYPES.includes(docType)) return sendError(res,422,'Invalid doc_type');
  if (!file) return sendError(res,422,'File upload failed or missing');
  if (!ALLOWED_MIME_TYPES.includes(file.mimetype)) return sendError(res,422,'Only JPG, PNG, or PDF files are allowed');
  try {
    const supabase=getSupabase();
    const {data:application,error:appErr}=await supabase.from('loan_applications').select('id,customer_id').eq('id',loanApplicationId).maybeSingle();
    if (appErr) throw appErr;
    if (!application || String(application.customer_id)!==String(customerId)) return sendError(res,404,'Loan application not found');
