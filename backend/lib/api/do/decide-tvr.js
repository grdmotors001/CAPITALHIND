...
export default async function handler(req,res){
  ...
  try{const {tvr_id,decision,remarks}=req.body||{}; if(!tvr_id)return sendError(res,422,'tvr_id is required'); if(!['verified','failed','hold'].includes(decision))return sendError(res,422,'decision must be verified, failed, or hold'); const s=getSupabase();
    ...
    const {data:updatedApp,error:ae}=await s.from('loan_applications').update({tvr_status:decision}).eq('id',t.loan_application_id).select('id,tvr_status,application_status').maybeSingle(); if(ae || !updatedApp)return sendError(res,500,'TVR decision saved, but application TVR gate could not be synchronized.');
