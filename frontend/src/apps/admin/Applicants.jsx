...
function flowState(app) {
  ...
  if (status === 'draft' || status === 'submitted' || status === 'fi_pending') return { current: 'FI', next: 'FI / Assignment' };
  ...
  if (['approved','sanctioned'].includes(status) && tvr === 'verified') return { current: 'Create Loan', next: 'Create Loan & Start EMI' };
  if (status === 'disbursed' || String(app.lifecycle_status || '').toLowerCase() === 'active') return { current: 'EMI Active', next: 'Collection / EMI' };
