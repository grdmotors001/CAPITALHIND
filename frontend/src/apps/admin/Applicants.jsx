...
function flowState(app) {
  const status = String(app.application_status || '').toLowerCase();
  const tvr = String(app.tvr_status || 'pending').toLowerCase();
  const lifecycle = String(app.lifecycle_status || '').toLowerCase();
  if (status === 'rejected') return { current: 'Application', next: 'Rejected', rejected: true };
  if (['draft','submitted','fi_pending'].includes(status)) return { current: 'FI', next: 'FI / Assignment' };
  if (status === 'fi_done') return { current: 'DO', next: 'DO / Approval' };
  if (['approved','sanctioned'].includes(status) && tvr !== 'verified') return { current: 'TVR', next: 'TVR Verification' };
  if (['approved','sanctioned'].includes(status) && tvr === 'verified' && lifecycle !== 'active') return { current: 'Create Loan', next: 'Create Loan & Start EMI' };
  if (status === 'disbursed' || lifecycle === 'active') return { current: 'EMI Active', next: 'Collection / EMI' };
  return { current: statusLabel(status), next: 'Next step' };
}
...
export default function Applicants(){
  ...
  const [search,setSearch]=useState(''),[view,setView]=useState(null),[editing,setEditing]=useState(null),[saving,setSaving]=useState(false);
  async function load(){setLoading(true);setError('');try{const d=await request('applicants');setRows(d.applicants||[])}catch(e){setError(e.message)}finally{setLoading(false)}}
  useEffect(()=>{load()},[]);
  const filtered=useMemo(()=>{const q=search.trim().toLowerCase();if(!q)return rows;return rows.filter(a=>[a.application_no,a.loan_account_no,a.customer?.full_name,a.customer?.phone,a.vehicle_no,a.dealer?.dealer_name,a.fi_executive_name].some(v=>String(v||'').toLowerCase().includes(q)))},[rows,search]);
  function startEdit(a){setEditing({id:a.id,customer:{...(a.customer||{})},loan:{application_no:a.application_no,application_status:a.application_status,vehicle_price:a.vehicle_price,down_payment:a.down_payment,loan_amount_requested:a.loan_amount_requested,tenure_months:a.tenure_months,vehicle_no:a.vehicle_no||'',chassis_no:a.chassis_no||'',ledger_no:a.ledger_no||'',file_no:a.file_no||'',file_record_no:a.file_record_no||'',cheques_qty:a.cheques_qty||0,fi_executive_name:a.fi_executive_name||'',fi_status:a.fi_status||'',case_status:a.case_status||'active',interest_rate:a.interest_rate||'',interest_amount:a.interest_amount||'',principal_amount:a.principal_amount||'',emi_no:a.emi_no||'',emi_amount:a.emi_amount||'',disbursement_date:a.disbursement_date||'',disbursed_amount:a.disbursed_amount||'',approved_by:a.approved_by||'',sanction_date:a.sanction_date||'',approval_valid_until:a.approval_valid_until||'',cibil_score:a.cibil_score||''}});setView(null);setMessage('');setError('')}
  const setC=(k,v)=>setEditing(e=>({...e,customer:{...e.customer,[k]:v}}));
  const setL=(k,v)=>setEditing(e=>({...e,loan:{...e.loan,[k]:v}}));
  async function save(e){e.preventDefault();setSaving(true);setError('');setMessage('');try{await request('applicants',{method:'POST',body:editing});setMessage('Applicant updated successfully.');setEditing(null);await load()}catch(e){setError(e.message)}finally{setSaving(false)}}
  return <div className="admin-page">
    <div className="admin-page-head"><div><div className="admin-eyebrow">APPLICATION REGISTER</div><h1>Applicants</h1><p>All created applicants — view complete application details or edit from a popup.</p></div><div className="admin-count">{rows.length} applicants</div></div>
